...
import { plans, getPlan } from '../data/plans.jsx'
...
export default function PlanDetail() {
  ...
  const others = plans.filter((p) => p.slug !== slug).slice(0, 8)
      ...
      <section className="relative px-4 pt-14 pb-16 md:pt-20 md:pb-20 bg-gradient-to-br from-orange-50 via-white to-rose-50 overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          <nav className="flex items-center gap-2 text-sm text-black mb-8">
            ...
            <Link to="/plans" className="hover:text-orange-600">MLM Plans</Link>
            <FiChevronRight className="text-slate-300" />
            <span className="text-slate-800 font-medium truncate">{plan.name}</span>
          </nav>
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-6">
            ...
            <div>
              <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight text-slate-900 mb-3">{plan.name}</h1>
              <p className="text-lg text-black max-w-2xl">{plan.tagline}</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-4 mt-8">
            ...
            <Link to="/plans" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl font-semibold text-slate-800 bg-white border border-slate-200 hover:border-orange-300 hover:text-orange-600 transition">
              <FiArrowLeft /> All Plans
            </Link>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto grid lg:grid-cols-3 gap-12">
          ...
          <aside className="lg:col-span-1">
            <div className="rounded-2xl border border-slate-200 p-6 sticky top-28">
              <h3 className="text-sm font-bold uppercase tracking-wider text-black mb-4">Other Plans</h3>
              <ul className="space-y-1">
                {others.map((p) => (
                  <li key={p.slug}>
                    <Link to={`/plans/${p.slug}`} className="group flex items-center gap-3 py-2 text-black hover:text-orange-600 transition">
